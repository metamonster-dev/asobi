<?php
$title = "공지사항";
include_once("./inc/head.php");
include_once("./inc/head_style06.php");
?>

<article class="sub_pg sub_bg sub_bg3">
    <div class="container-fluid pt-3 pb-5">
        <form action="">
            <div class="m_top">
                <div class="input-group">
                    <input type="date" class="form-control col-5">
                    <div class="ip_sch_wr col-7 px-0">
                        <input type="search" class="form-control ip_search">
                        <button type="submit" class="btn btn_sch btn_sch2"></button>
                    </div>
                </div>
                <div class="m_top_ico">
                    <img src="./img/m3_top.png">
                </div>
            </div>
            <ul class="note_list pb-5">
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[본사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[지사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                        <div class="rect rect2">
                            <img src="./img/sample_img3.jpg">
                        </div>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[본사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                    </a>
                </li> 
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[지사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                        <div class="rect rect2">
                            <img src="./img/sample_img3.jpg">
                        </div>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[본사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[지사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                        <div class="rect rect2">
                            <img src="./img/sample_img3.jpg">
                        </div>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[본사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[지사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                        <div class="rect rect2">
                            <img src="./img/sample_img3.jpg">
                        </div>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[본사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                    </a>
                </li>
                <li>
                    <a href="./institute_notice_detail1.php">
                        <p class="text-dark_gray fs_13 fw_300 mb-3"><span class="text-primary fw_500 mr-2">[지사공지]</span> 2023.04.01 월요일</p>
                        <h4 class="tit_h4 mb-3">공지사항 제목 입니다.</h4>
                        <p class="line2_text line_h1_4">공지사항 내용입니다. 두 줄까지 노출 공지사항 내용입니다. 두 줄까지 노출</p>
                        <div class="rect rect2">
                            <img src="./img/sample_img3.jpg">
                        </div>
                    </a>
                </li>
            </ul>

        </form>
        
        <!-- 지사/본사 표시 -->
        <!-- <div class="f_btn_wr">
            <button type="button" class="btn float_btn" onclick="location.href='./institute_notice_write.php'"><img src="./img/ic_write.png" style="width: 3rem;"></button>
        </div> -->
    </div>
</article>


<?php include_once("./inc/tail.php"); ?>